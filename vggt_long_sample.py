...
class VGGT_Long:
    ...
    def run(self):
        ...
        sample_rate = self.config['Model'].get('sample_rate', 1)  # 默认不采样
        if sample_rate > 1:
            sampled_list = self.img_list[::sample_rate]
            print(f"[Sampling Enabled] Using every {sample_rate}th image "
                f"→ {len(sampled_list)} / {len(self.img_list)} images kept.")
            self.img_list = sampled_list
        # print(self.img_list)
